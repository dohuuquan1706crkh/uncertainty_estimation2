...
from src.data.camus.config import CamusTags, Label, in_channels, View
from src.data.camus.dataset import Camus
from src.data.config import DataParameters, Subset
from src.data.data_module import VitalDataModule
from src.data.mixins import StructuredDataMixin
...
class CamusDataModule(StructuredDataMixin, VitalDataModule):
        ...
        **kwargs,
        ...
        super().__init__(
            data_params=DataParameters(
                ...
                out_shape=(output_channels, *image_shape),
                labels=labels,
            ),
            **kwargs,
        )
    ...
    def setup(self, stage: Literal["fit", "test"]) -> None:  # noqa: D102
        if stage == "fit":
            ...
            self._dataset[Subset.VAL] = Camus(
                image_set=Subset.VAL, **self._dataset_kwargs
            )
        if stage == "test":
            self._dataset[Subset.TEST] = Camus(
                image_set=Subset.TEST, predict=True, **self._dataset_kwargs
            )

    def group_ids(
        self, subset: Subset, level: Literal["patient", "view"] = "view"
    ) -> List[str]:
        ...
        """
        subset_data = self.dataset().get(
            subset, Camus(image_set=subset, **self._dataset_kwargs)
        )
