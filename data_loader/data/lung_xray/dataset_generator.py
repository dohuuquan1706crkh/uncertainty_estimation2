...
from src.data.config import Tags, Subset
from src.data.lung_xray.config import img_save_options, seg_save_options
    ...
    for _, file in tqdm(enumerate(file_list), total=len(file_list)):
        ...
        if os.path.isfile(gt_path):
            ...
            sample_group.create_dataset(
                name=Tags.img, data=img_resize, **img_save_options
            )
            sample_group.create_dataset(
                name=Tags.gt, data=gt_resize, **seg_save_options
            )
...
def write_hdf5(raw_data_path, h5_name, test_split, val_split, img_size, split_source):
    ...
    write_group_hdf5(
        f.create_group(Subset.TRAIN.value),
        train_files,
        xray_folder_path,
        gt_folder_path,
        img_size,
    ...
    write_group_hdf5(
        f.create_group(Subset.VAL.value),
        val_files,
        xray_folder_path,
        gt_folder_path,
        img_size,
    ...
    write_group_hdf5(
        f.create_group(Subset.TEST.value),
        test_files,
        xray_folder_path,
        gt_folder_path,
        img_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "data_path", type=str, help="Root directory under which the raw data is stored"
    )
    parser.add_argument(
        "--h5_name",
        type=str,
        default="lung_xray.h5",
    ...
    parser.add_argument(
        "--val_split", type=float, help="Validation set split", default=0.1
    )
    parser.add_argument(
        "--split_source",
        action="store_true",
        help="Split train test split with image origin",
    )
    parser.add_argument(
        "--img_size", type=int, help="Size of resized images", default=256
    )
