...
from src.data.config import Subset, Tags
from src.data.lung_xray.config import View
from src.data.transforms import DiffusedNoise
...
class LungXRay(VisionDataset):
    ...
    def list_items(self) -> List[str]:
        ...
        image_sets = (
            ...
            else [set.value for set in Subset]
        )
        with h5py.File(self.root, "r") as dataset:
            ...
            items = (
                items[: self.max_patients] if self.max_patients is not None else items
            )
...
if __name__ == "__main__":
    ...
    ds = LungXRay(
        Path(params.path), image_set=Subset.TEST, predict=params.predict, test_all=True
    )
