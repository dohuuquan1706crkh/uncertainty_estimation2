...
from src.data.lung_xray.config import Label
from src.data.lung_xray.dataset import LungXRay
...
class LungXRayDataModule(VitalDataModule):
    ...
    def setup(self, stage: Literal["fit", "test"]) -> None:  # noqa: D102
        if stage == "fit":
            self._dataset[Subset.TRAIN] = LungXRay(
                image_set=Subset.TRAIN,
                **self._dataset_kwargs,
                max_patients=self.max_patients,
                data_augmentation=self.data_augmentation,
            )
            self._dataset[Subset.VAL] = LungXRay(
        ...
        if stage == "test":
            ...
            self._dataset[Subset.TEST] = LungXRay(
