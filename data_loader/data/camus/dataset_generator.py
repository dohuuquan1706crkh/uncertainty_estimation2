...
from src.data.camus.config import (
    CamusTags,
    FullCycleInstant,
    Instant,
    Label,
...
from src.data.camus.utils.register import CamusRegisteringTransformer
from src.data.config import Subset
from src.utils.image.io import load_mhd
from src.utils.image.transform import remove_labels, resize_image
...
class CrossValidationDatasetGenerator:
    ...
    _subset_names_in_data: Dict[
        Subset, Literal["training", "validation", "testing"]
    ] = {
    ...
    def __call__(
        ...
        self.labels_to_remove = (
            [] if labels is None else [label for label in Label if label not in labels]
        )
        ...
        self.sequence_type_instants = asdict(
            Instant.from_sequence_type(sequence_type)
        ).values()
        ...
        with h5py.File(output, "w") as dataset:
            ...
            for fold in folds:
                ...
                for subset, subset_name_in_data in self._subset_names_in_data.items():
                    fold_subset_patients = self.get_fold_subset_from_file(
                        data, fold, subset_name_in_data
                    )
                    fold_group.create_dataset(
                        subset.value, data=np.array(fold_subset_patients, dtype="S")
                    )
            ...
            patient_ids = reduce(
                add,
                [
                    self.get_fold_subset_from_file(data, fold, subset)
                    for subset in self._subset_names_in_data.values()
                ],
            )
            patient_ids.sort()
            ...
            for patient_id in tqdm(
                patient_ids,
                unit="patient",
                desc=f"Writing patient data to HDF5 file: {output}",
            ):
    ...
    def _write_patient_data(self, patient_group: h5py.Group) -> None:
        ...
        patient_dir = f"patient{int(float(patient_id)):04d}"
        available_views = [
            view
            ...
            if (
                self.data
                / patient_dir
                / self.info_filename_format.format(patient=patient_id, view=view)
            ).exists()
        ]
        for view in available_views:
            ...
            data_y = remove_labels(
                data_y,
                [lbl.value for lbl in self.labels_to_remove],
                fill_label=Label.BG.value,
            )
            if self.flags[CamusTags.registered]:
                registering_parameters, data_y_proc, data_x_proc = (
                    self.registering_transformer.register_batch(data_y, data_x)
                )
            else:
                data_x_proc = np.array(
                    [
                        resize_image(x, self.target_image_size, resample=LINEAR)
                        for x in data_x
                    ]
                )
                data_y_proc = np.array(
                    [resize_image(y, self.target_image_size) for y in data_y]
                )

            # Write image and groundtruth data
            patient_view_group = patient_group.create_group(view)
            patient_view_group.create_dataset(
                name=CamusTags.img_proc,
                data=data_x_proc[..., np.newaxis],
                **img_save_options,
            )
            patient_view_group.create_dataset(
                name=CamusTags.gt, data=data_y, **seg_save_options
            )
            patient_view_group.create_dataset(
                name=CamusTags.gt_proc, data=data_y_proc, **seg_save_options
            )
    ...
    def _get_view_data(
    ...
    ) -> Tuple[np.ndarray, np.ndarray, List[Real], Dict[str, int]]:
        ...
        with open(str(view_info_fn), "r") as view_info_file:
            view_info = {
                (pair := line.split(": "))[0]: pair[1]
                for line in view_info_file.read().splitlines()
            }
        for instant in self.sequence_type_instants:
            ...
            instants[instant] = (
                int(
                    view_info[
                        instant if instant != FullCycleInstant.ED_E else "NbFrame"
                    ]
                )
                - 1
            )
    ...
    def _get_sequence_data(
        ...
        """
        patient_dir = f"patient{int(float(patient_id)):04d}"
        patient_folder = self.data / patient_dir
        sequence_fn_template = f"{patient_dir}_{view}_half_sequence{{}}.nii.gz"
        ...
        for image, segmentation in zip(
            sequence, sequence_gt
        ):  # For every instant in the sequence
            data_x.append(image)
...
def main():
    ...
    parser.add_argument(
        ...
        "--register",
        action="store_true",
        help="Apply registering on images and groundtruths",
    )
    parser.add_argument(
        "--labels",
