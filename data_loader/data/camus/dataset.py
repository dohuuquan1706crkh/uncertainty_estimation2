...
from data_loader.data.camus.config import CamusTags, Label, View
from data_loader.data.camus.data_struct import PatientData, ViewData
from data_loader.data.camus.utils.measure import EchoMeasure
from data_loader.data.camus.utils.register import CamusRegisteringTransformer
from data_loader.data.config import Subset
from data_loader.decorators import squeeze
from data_loader.transform import remove_labels, segmentation_to_tensor
...
class Camus(VisionDataset):
    ...
    def list_groups(self, level: Literal["patient", "view"] = "view") -> List[str]:
        ...
        with h5py.File(self.root, "r") as dataset:
            ...
            groups = [
                ...
                for patient_path_byte in dataset[
                    f"cross_validation/fold_{self.fold}/{self.image_set.value}"
                ]
            ]
            groups = (
                groups[: self.max_patients] if self.max_patients is not None else groups
            )
            if level == "view":
                groups = [
                    f"{patient}/{view}"
    ...
    def _get_instant_paths(self) -> List[ItemId]:
        ...
        def include_image(view_group: h5py.Group, instant: int) -> bool:
            is_clinically_important_instant = instant in (
                view_group.attrs[instant_key]
                for instant_key in view_group.attrs[CamusTags.instants]
            )
            return (
        ...
        with h5py.File(self.root, "r") as dataset:
            for view_path in view_paths:
                ...
                for instant in range(view_group[CamusTags.gt_proc].shape[0]):
    ...
    def _get_train_item(self, index: int) -> RecursiveInstantItem:
        ...
        if self.neighbors:
            ...
            if isinstance(
                self.neighbors, int
            ):  # If `neighbors` indicates the number of neighbors on each side
                instant_diffs = itertools.chain(
                    range(-self.neighbors, 0), range(1, self.neighbors + 1)
                )
            ...
            if self.neighbor_padding == "edge":
                neigh_instants = {
                    diff: np.clip(instant + diff, 0, seq_len) for diff in instant_diffs
                }
            elif self.neighbor_padding == "wrap":
                neigh_instants = {
                    diff: (instant + diff) % seq_len for diff in instant_diffs
                }
            else:
                raise ValueError(
            ...
            item[CamusTags.neighbors] = {
                diff: self._get_instant_item(
                    self.item_list.index((patient_view_key, instant))
                )
    ...
    def _get_instant_item(self, index: int) -> InstantItem:
        ...
        # Collect data
        with h5py.File(self.root, "r") as dataset:
            view_imgs, view_gts, view_orig_gt = self._get_data(
                dataset,
                patient_view_key,
                CamusTags.img_proc,
                CamusTags.gt_proc,
            ...
            for view in dataset[self.item_list[index]]:
                if view in self.views:
                    ...
                    if self.dataset_with_sequence and not self.use_sequence:
                        ...
                        instants = {
                            instant: idx for idx, instant in enumerate(instants)
                        }
                    ...
                    if self.registered_dataset:
                        registering_parameters = {
                            reg_step: Camus._get_metadata(
                                dataset, patient_view_key, reg_step
                            )
                    ...
                    attrs = {
                        CamusTags.frame_pos: torch.linspace(
                            0, 1, steps=len(proc_imgs)
                        ).unsqueeze(1),
                    ...
                    if (
                        len(self.labels) == 2
                    ):  # For binary segmentation, make foreground class 1.
    ...
    @squeeze
    def _get_data(
        file: h5py.File, patient_view_key: str, *data_tags: str
    ) -> List[np.ndarray]:
    ...
    @squeeze
    def _get_metadata(
        file: h5py.File, patient_view_key: str, *metadata_tags: str
    ) -> List[np.ndarray]:
...
def get_segmentation_attributes(
    ...
    if Label.LV in labels:
        attrs.update(
            {
                CamusTags.lv_area: EchoMeasure.structure_area(
    ...
    if Label.MYO in labels:
        attrs.update(
            {
                CamusTags.myo_area: EchoMeasure.structure_area(
    ...
    if Label.ATRIUM in labels:
        attrs.update(
            {
                CamusTags.atrium_area: EchoMeasure.structure_area(
...
if __name__ == "__main__":
    ...
    from data_loader.data.camus.config import View
    ...
    if params.predict:
        ...
        print("Image shape: {}".format(img.shape))
        print("Image min: {}".format(img.min()))
        print("Image max: {}".format(img.max()))
        print("GT shape: {}".format(gt.shape))
