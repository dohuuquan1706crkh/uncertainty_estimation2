...
from data_loader.data.camus.config import Label
from data_loader.data.config import SemanticStructureId
from data_loader.decorators import auto_cast_data, batch_function
from data_loader.measure import Measure, T
...
class EchoMeasure(Measure):
    ...
    def _lv_base(
        ...
        dilated_myocardium = ndimage.binary_dilation(
            myocardium, structure=EchoMeasure._square_3x3_connectivity
        )
        dilated_others = ndimage.binary_dilation(
            others, structure=EchoMeasure._square_3x3_connectivity
        )
        y_coords, x_coords = np.where(
            left_ventricle * dilated_myocardium * dilated_others
        )
        ...
        return (
            (
                y_coords[left_half_mask][bottom_left_point_idx],
                x_coords[left_half_mask][bottom_left_point_idx],
            ),
            (
                y_coords[~left_half_mask][bottom_right_point_idx],
                x_coords[~left_half_mask][bottom_right_point_idx],
            ),
        )
    ...
    def lv_base_width(
        ...
        """
        lv_base_coords = EchoMeasure._lv_base(
            segmentation, lv_labels=lv_labels, myo_labels=myo_labels
        )
        return (
    ...
    def lv_length(
        ...
        left_ventricle = np.isin(segmentation, lv_labels)
        lv_apex_coords = np.unravel_index(
            np.argmax(dist_to_lv_base_mid * left_ventricle), segmentation.shape
        )
