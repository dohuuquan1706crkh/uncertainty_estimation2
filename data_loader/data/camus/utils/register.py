...
from data_loader.numpy import to_categorical
from data_loader.measure import Measure
from data_loader.affine import (
    AffineRegisteringTransformer,
    Crop,
    Rotation,
    Shift,
)
...
class CamusRegisteringTransformer(AffineRegisteringTransformer):
    ...
    def _compute_shift_parameters(self, segmentation: np.ndarray) -> Shift:
        ...
        distance_from_left_atrium_to_border = (
            segmentation.shape[0] - segmentation_props.bbox[2]
        )
        ...
        epicardium_center = Measure.structure_center(
            to_categorical(segmentation), [Label.LV.value, Label.MYO.value]
        )

        # Center the image as closely as possible around the epicardium without cutting off the left atrium
        rows_shift = max(
            epicardium_center[0] - segmentation_center[0],
            -distance_from_left_atrium_to_border,
        )
    ...
    def _compute_crop_parameters(
        ...
        bbox_coord = Measure.bbox(
            segmentation, list(range(1, self.num_classes)), bbox_margin=margin
        )
