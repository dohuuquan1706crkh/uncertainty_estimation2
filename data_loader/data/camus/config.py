...
from data_loader.data.config import DataTag, Tags
...
class Instant:
    ...
    @classmethod
    def from_sequence_type(
        cls, sequence_type: Literal["half_cycle", "full_cycle"]
    ) -> "Instant":
...
class CamusTags(Tags):
    ...
    def list_available_attrs(cls, labels: Sequence[Label]) -> List[str]:
        ...
        if Label.LV in labels and Label.MYO in labels:
            attrs.extend(
                [cls.lv_base_width, cls.lv_length, cls.epi_center_x, cls.epi_center_y]
            )
