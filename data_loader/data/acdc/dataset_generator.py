...
from src.data.acdc.config import AcdcTags, Instant, Label, image_size
from src.data.acdc.utils.register import AcdcRegisteringTransformer
from src.data.config import Subset
from src.utils.format.numpy import to_categorical, to_onehot
from src.utils.image.measure import Measure
from src.utils.image.transform import centered_resize
...
def extract_image_paths(root_dir: Path) -> List[Path]:
    ...
    paths = [
        Path(path) for path in paths if "Info" not in path and path.find("_4d") < 0
    ]
    return paths


def _generate_centered_prob_map(
    image: np.ndarray, shape: np.ndarray, center: np.ndarray, label: int
) -> np.array:
...
def generate_probability_map(
    dataset: h5py.File, group: h5py.Group, data_augmentation: bool
):
    ...
    """
    patient_keys = (
        [key for key in group.keys() if key.endswith("_0")]
    ...
    images_center = [
        Measure.structure_center(img, 3).astype(np.int16) for img in images
    ]
    ...
    for label in Label:
        prob_map = np.array(
            [
                _generate_centered_prob_map(
                    np.copy(img), prior_shape, center, label.value
                )
                for img, center in tqdm(
                    zip(images, images_center), desc=str(label), total=len(images)
                )
            ]
        )
...
def load_instant_data(
    img_path: Path, gt_path: Optional[Path] = None
...
def write_instant_group(
    ...
    if registering_transformer is not None:
        registering_parameters, gt_data, r_img = registering_transformer.register_batch(
            gt_data, r_img
        )
...
def create_database_structure(
    ...
    for rotation in rotations:
        ...
        write_instant_group(
            patient.create_group(Instant.ED.value),
            ed_img,
            edg_img,
            ed_voxel_size,
            rotation,
        ...
        write_instant_group(
            patient.create_group(Instant.ES.value),
            es_img,
            esg_img,
            es_voxel_size,
            rotation,
...
def generate_dataset(
    ...
    if data_augmentation:
        logger.info(
            "Data augmentation enabled, rotation " "from -60 to 60 by step of 15."
        )
    if registering:
    ...
    if os.path.exists(os.path.join(data_path, "testing_with_gt_mid")):
        ...
        test_paths = np.array(
            [np.insert(i, 2, [None, None]).tolist() for i in test_paths]
        )
    ...
    with h5py.File(output_path, "w") as output_dataset:
        ...
        for p_ed, g_ed, p_es, g_es in tqdm(train_paths, desc="Training"):
            ...
            if str(p_ed) != str(g_ed).replace("_gt", "") or str(p_es) != str(
                g_es
            ).replace("_gt", ""):
                raise ValueError(
            ...
            create_database_structure(
                group, data_augmentation, registering, p_ed, g_ed, p_es, g_es
            )
        ...
        for p_ed, g_ed, p_es, g_es in tqdm(valid_paths, desc="Validation"):
            ...
            if str(p_ed) != str(g_ed).replace("_gt", "") or str(p_es) != str(
                ...
                raise ValueError(
                    f"File name don't match: {p_ed} instead of {p_ed}, {p_es} instead of {g_es}."
                )
        ...
        for p_ed, g_ed, p_mid, g_mid, p_es, g_es in tqdm(test_paths, desc="Testing"):
            ...
            # Find missmatch in the zip
            if str(basename(p_ed)) != str(basename(g_ed)).replace("_gt", "") or str(
                basename(p_es)
            ) != str(basename(g_es)).replace("_gt", ""):
                raise ValueError(
                    f"File name don't match: {p_ed} instead of {g_ed}, {p_es} instead of {g_es}."
                )
            create_database_structure(
...
def main():
    ...
    parser.add_argument(
        "data_path",
        type=Path,
        help="Path to the root directory of the downloaded ACDC data",
    )
    parser.add_argument(
        "--output_path",
        type=Path,
        default="acdc.h5",
        help="Path to the HDF5 file to generate",
    )
    ...
    data_processing_group.add_argument(
        ...
        "--data_augmentation",
        action="store_true",
        help="Add data augmentation (rotation -60 to 60).",
    )
    data_processing_group.add_argument(
        "-r",
        "--registering",
    ...
    generate_dataset(
        args.data_path, args.output_path, args.data_augmentation, args.registering
    )
