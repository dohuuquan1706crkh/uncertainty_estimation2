...
from src.data.acdc.config import Label
from src.utils.image.register.affine import (
...
class AcdcRegisteringTransformer(AffineRegisteringTransformer):
    ...
    def _compute_shift_parameters(self, segmentation: np.ndarray) -> Shift:
        ...
        lv_myo_center = np.array(
            self._find_structure_center(segmentation, [Label.LV.value, Label.MYO.value])
        )
    ...
    def _compute_rotation_parameters(self, segmentation: np.ndarray) -> Rotation:
        ...
        rv_center = np.array(
            self._find_structure_center(
                segmentation, Label.RV.value, default_center=tuple(lv_myo_center)
            )
        )
