...
from src.data.acdc.config import AcdcTags, Instant, image_size
from src.data.acdc.data_struct import InstantData, PatientData
...
from src.utils.decorators import squeeze
...
class Acdc(VisionDataset):
    ...
    def list_groups(
        self, level: Literal["patient", "instant"] = "instant"
    ) -> List[str]:
        ...
        with h5py.File(self.root, "r") as dataset:
            ...
            if level == "instant":
                groups = [
                    ...
                    for instant in dataset[patient].keys()
                ]
    ...
    def _get_train_item(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        with h5py.File(self.root, "r") as dataset:
            ...
            patient_imgs, patient_gts = self._get_data(
                dataset, set_patient_instant_key, AcdcTags.img, AcdcTags.gt
            )

            img = patient_imgs[slice]
            gt = patient_gts[slice]

            voxel = Acdc._get_metadata(
                dataset, set_patient_instant_key, AcdcTags.voxel_spacing
            )
    ...
    def _get_test_item(self, index: int) -> PatientData:
        ...
        with h5py.File(self.root, "r") as dataset:
            ...
            for instant in dataset[patient_key]:
                ...
                # Collect and process data
                imgs, gts = Acdc._get_data(
                    dataset, patient_instant_key, AcdcTags.img, AcdcTags.gt
                )
                ...
                if self.registered_dataset:
                    registering_parameters = {
                        reg_step: Acdc._get_metadata(
                            dataset, patient_instant_key, reg_step
                        )
                        for reg_step in AcdcRegisteringTransformer.registering_steps
                    }

                voxel = Acdc._get_metadata(
                    dataset, patient_instant_key, AcdcTags.voxel_spacing
                )
                ...
                #     voxelspacing=voxel,
                # )
                patient_data.instants[instant] = (imgs, gts)
        return patient_data

    @staticmethod
    @squeeze
    def _get_data(
        file: h5py.File, set_patient_instant_key: str, *data_tags: str
    ) -> List[np.ndarray]:
    ...
    @squeeze
    def _get_metadata(
        file: h5py.File, set_patient_instant_key: str, *metadata_tags: str
    ) -> List[np.ndarray]:
    ...
    def get_normalized_slice(gt: np.ndarray, instant: int, image_size: int) -> float:
        ...
        if (
            np.sum(np.equal(gt[0, :, :], 0)) != image_size * image_size
        ):  # if first slice is not 0, add i virtually
            slice_index += 1
            number_slices += 1
