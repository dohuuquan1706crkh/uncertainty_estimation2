...
from src.data.camus.config import CamusTags
from src.data.camus.data_struct import PatientData, ViewData
from src.data.config import Subset
from src.data.transforms import NormalizeSample
from src.utils.decorators import squeeze
from src.utils.image.transform import segmentation_to_tensor
...
class HMC_QU(VisionDataset):
    ...
    def _get_train_item(self, index: int) -> Dict[str, Union[str, Tensor]]:
        ...
        with h5py.File(self.root, "r") as dataset:
            ...
            view_imgs, view_gts = self._get_data(
                dataset,
                f"{self.image_set}/{patient_view_key}",
                CamusTags.img_proc,
                CamusTags.gt_proc,
            )
    ...
    def _get_test_item(self, index: int) -> PatientData:
        ...
        with h5py.File(self.root, "r") as dataset:
            ...
            for view in dataset[f"{self.image_set}/{self.item_list[index]}"]:
                ...
                proc_imgs_tensor = torch.stack(
                    [to_tensor(proc_img) for proc_img in proc_imgs]
                )
                proc_gts_tensor = torch.stack(
                    [segmentation_to_tensor(proc_gt) for proc_gt in proc_gts]
                )
...
if __name__ == "__main__":
    ...
    from src.data.camus.config import View
    ...
    ds = HMC_QU(
        Path(params.path), image_set=Subset.TRAIN, predict=params.predict
    )  # , transform=Normalize(mean=0.1555, std=0.2126))
