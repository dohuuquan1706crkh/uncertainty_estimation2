...
from src.data.camus.config import Label

from src.data.config import DataParameters, Subset
from src.data.data_module import VitalDataModule
from src.data.HMC_QU.config import image_size, in_channels
from src.data.HMC_QU.dataset import HMC_QU
...
class HMC_QUDataModule(VitalDataModule):
    ...
    def setup(self, stage: Literal["fit", "test"]) -> None:  # noqa: D102
        if stage == "fit":
            self._dataset[Subset.TRAIN] = HMC_QU(
                image_set=Subset.TRAIN, **self._dataset_kwargs
            )
            self._dataset[Subset.VAL] = HMC_QU(
