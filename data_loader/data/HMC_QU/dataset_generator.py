...
from src.data.camus.config import CamusTags, View, img_save_options, seg_save_options
from src.data.config import Subset
from src.data.HMC_QU.config import image_size
from src.utils.image.transform import resize_image
...
def generate_dataset(
    path: Path, name: Path, seed: int, test_size: float, val_size: float
) -> None:
    ...
    dataset_info = dataset_info[
        dataset_info[AVAILABLE_GT_ROW] == AVAILABLE_GT_SYMBOL
    ]  # Only keep samples with GT
    patient_list = list(dataset_info.index.values)

    train_patients, test_patients = train_test_split(
        patient_list, test_size=test_size, random_state=seed
    )
    train_patients, val_patients = train_test_split(
        train_patients, test_size=val_size, random_state=seed
    )
...
def generate_set(
    patient_list: List[str], path: Path, dataset_info: pd.DataFrame, group: h5py.Group
) -> None:
    """Generates on set.
    ...
    for patient in patient_list:
        ...
        gt = scipy.io.loadmat(
            str(path / GT_RELATIVE_PATH / ("Mask_" + patient + ".mat"))
        )["predicted"]
        ...
        img = np.array(
            [
                color.rgb2gray(
                    resize_image(x, (image_size, image_size), resample=LINEAR)
                )
                for x in img
            ]
        )
        ...
        patient_view_group.create_dataset(
            name=CamusTags.img_proc, data=img, **img_save_options
        )
        patient_view_group.create_dataset(
            name=CamusTags.gt_proc, data=gt_proc, **seg_save_options
        )
        patient_view_group.create_dataset(
            name=CamusTags.gt, data=gt, **seg_save_options
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path of the HMC_QU downloaded dataset unziped folder.",
    )
    parser.add_argument(
        "--name",
        type=Path,
        default=Path("HMC_QU.h5"),
        help="Name of the generated hdf5 file.",
    )
    parser.add_argument("--seed", type=int, default=0, help="Seed for the data split")
    parser.add_argument(
        "--test_size", type=float, default=0.25, help="Size of test set"
    )
    parser.add_argument(
        "--val_size", type=float, default=0.1, help="Size of validation set"
    )
