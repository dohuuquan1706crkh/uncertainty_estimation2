...
from data_loader.format.native import Item
from data_loader.format.native import prefix as prefix_fn
from data_loader.format.native import squeeze as squeeze_fn
...
def squeeze(
    fn: Callable[..., Sequence[Item]]
) -> Callable[..., Union[Item, Sequence[Item]]]:
    ...
    def batch_function_decorator(func: Callable) -> Callable:
        ...
        def _loop_function_on_batch(*args, **kwargs):
            ...
            if data.ndim == item_ndim:  # If the input data is a single item
                ...
                if (
                    result.ndim == 0
                ):  # If the function's output is a single number, add a dim of 1 for consistency
                    result = result[None]
            elif data.ndim == (item_ndim + 1):  # If the input data is a batch of items
                result = np.array(
                    [func(*self_or_empty, item, *args, **kwargs) for item in data]
                )
            else:
                raise RuntimeError(
