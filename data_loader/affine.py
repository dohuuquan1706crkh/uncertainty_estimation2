...
from data_loader.numpy import to_categorical, to_onehot
from data_loader.transform import resize_image
...
class AffineRegisteringTransformer:
    ...
    ) -> Dict[str, RegisteringParameter]:
        return {
            "shift": (0, 0),
            "rotation": 0,
            "zoom": (1, 1),
            "crop:": segmentation.shape
            + (0, 0, segmentation.shape[0] - 1, segmentation.shape[1] - 1),
        }
    ...
    def register_batch(
        ...
        for idx, (segmentation, image) in enumerate(
            itertools.zip_longest(segmentations, images)
        ):
            if image is not None:
                (
                    segmentation_registering_parameters,
                    registered_segmentation,
                    registered_image,
                ) = self.register(segmentation, image)
                registered_images.append(registered_image)
            else:
                segmentation_registering_parameters, registered_segmentation = (
                    self.register(segmentation)
                )
            ...
            for registering_parameter, values in registering_parameters.items():
                values.append(
                    segmentation_registering_parameters[registering_parameter]
                )
    ...
    def register(
        ...
        segmentation, original_segmentation_format = self._check_segmentation_format(
            segmentation
        )
        if image is not None:
        ...
        for registering_step in self.registering_steps:
            ...
            if image is None:
                registering_step_parameters, segmentation = registering_step_fct(
                    segmentation
                )
            else:
                registering_step_parameters, segmentation, image = registering_step_fct(
                    segmentation, image
                )
        ...
        segmentation = self._restore_segmentation_format(
            segmentation, original_segmentation_format
        )
    ...
    def undo_batch_registering(
        ...
        registering_parameters: Mapping[str, np.ndarray],
    ) -> np.ndarray:
        ...
        if "crop" in self.registering_steps:
            ...
            unregistered_segmentations = np.empty(
                (len(segmentations), *registering_parameters["crop"][0][:2])
            )
        else:
        ...
        for idx, segmentation in enumerate(segmentations):
            seg_registering_parameters = {
                registering_step: values[idx]
                for registering_step, values in registering_parameters.items()
            }
            unregistered_segmentations[idx] = self.undo_registering(
                segmentation, seg_registering_parameters
            )
    ...
    def undo_registering(
        self,
        segmentation: np.ndarray,
        registering_parameters: Mapping[str, RegisteringParameter],
    ) -> np.ndarray:
        ...
        registering_parameters.update(
            {
                registering_step: self._get_default_parameters(segmentation)[
                    registering_step
                ]
        ...
        if "crop" in self.registering_steps:
            segmentation = self._restore_crop(
                segmentation, registering_parameters["crop"]
            )
        ...
        for registering_step in reversed(registering_steps_wo_crop):
            segmentation = self._transform_segmentation(
                segmentation, transformation_parameters[registering_step]
            )

        # Restore the segmentation to its original format
        return self._restore_segmentation_format(
            segmentation, original_segmentation_format
        )

    def _check_segmentation_format(
        ...
        if (
            is_categorical_2d or is_categorical_3d
        ):  # If the image is not already in categorical format
    ...
    @staticmethod
    def _restore_segmentation_format(
        segmentation: np.ndarray, format: Tuple[bool, bool]
    ) -> np.ndarray:
        ...
        if (
            is_categorical_2d or is_categorical_3d
        ):  # If the segmentation was originally categorical
    ...
    def _compute_zoom_to_fit_parameters(
        self, segmentation: np.ndarray, margin: float = 0.1
    ) -> Zoom:
    ...
    def _compute_crop_parameters(
        self, segmentation: np.ndarray, margin: float = 0.05
    ) -> Crop:
    ...
    def _center(
        self, segmentation: np.ndarray, image: None
    ) -> Tuple[Shift, np.ndarray]:
        pass

    def _center(
        self, segmentation: np.ndarray, image: np.ndarray = None
    ) -> Tuple[Shift, np.ndarray, np.ndarray]:
        ...
        out = pixel_shift_by_axis, self._transform_segmentation(
            segmentation, shift_parameters
        )
        if image is not None:
    ...
    def _rotate(
        ...
        out = rotation_angle, self._transform_segmentation(
            segmentation, rotation_parameters
        )
        if image is not None:
    ...
    def _zoom_to_fit(
        self, segmentation: np.ndarray, image: None
    ) -> Tuple[Zoom, np.ndarray]:
        pass

    def _zoom_to_fit(
        self, segmentation: np.ndarray, image: np.ndarray = None
    ) -> Tuple[Zoom, np.ndarray, np.ndarray]:
        ...
        out = zoom_to_fit, self._transform_segmentation(
            segmentation, zoom_to_fit_parameters
        )
        if image is not None:
    ...
    def _crop_resize(
        self, segmentation: np.ndarray, image: None
    ) -> Tuple[Crop, np.ndarray]:
        pass

    def _crop_resize(
        self, segmentation: np.ndarray, image: np.ndarray = None
    ) -> Tuple[Crop, np.ndarray, np.ndarray]:
        """Applies a zoom along each axis to fit the segmentation (and image) to the area of interest.
        ...
        def _crop(image: np.ndarray, bbox: tuple) -> np.ndarray:
            ...
            image = np.pad(
                image, (row_pad, col_pad), mode="constant", constant_values=0
            )
        ...
        if image is not None:
            ...
            image = resize_image(image, self.crop_shape[::-1], resample=LINEAR)[
                ..., np.newaxis
            ]
            out += (image,)
    ...
    def _restore_crop(
        self, segmentation: np.ndarray, crop_parameters: Crop
    ) -> np.ndarray:
        ...
        segmentation = to_onehot(
            resize_image(to_categorical(segmentation), bbox_shape[::-1]),
        ...
        og_segmentation[
            max(0, row_min) : min(row_max, og_shape[0]),
            max(0, col_min) : min(col_max, og_shape[1]),
            :,
        ] = segmentation[
            row_pad[0] : bbox_shape[0] - row_pad[1],
    ...
    def _transform_image(
        self,
        image: np.ndarray,
        transform_parameters: Mapping[str, RegisteringParameter],
    ) -> np.ndarray:
    ...
    def _transform_segmentation(
        ...
        """
        segmentation = self.transformer.apply_transform(
            segmentation, transform_parameters
        )
