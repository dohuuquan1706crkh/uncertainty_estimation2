...
class Measure:
    ...
    def structure_center(
        segmentation: T, labels: SemanticStructureId, axis: int = None
    ) -> T:
        ...
        if any(
            np.isnan(center)
        ):  # Default to the center of the image if the center of mass can't be found
    ...
    @auto_cast_data
    @batch_function(item_ndim=2)
    def structure_orientation(
        segmentation: T, labels: SemanticStructureId, reference_orientation: int = 0
    ) -> T:
    ...
    def bbox(
        ...
        if normalize:
            ...
            roi_bbox[[0, 2]] = (
                roi_bbox[[0, 2]] / segmentation.shape[0]
            )  # Normalize height
            roi_bbox[[1, 3]] = (
                roi_bbox[[1, 3]] / segmentation.shape[1]
            )  # Normalize width
    ...
    def denormalize_bbox(
        roi_bbox: T, output_size: Tuple[int, int], check_bounds: bool = False
    ) -> T:
