...
def prefix(
    map: Mapping[str, Any], prefix: str, exclude: Union[str, Sequence[str]] = None
) -> Dict[str, Any]:
...
def filter_excluded(
    seq: Sequence[str], to_exclude: Sequence[Union[str, Path]]
) -> List[str]:
    ...
    for list_items_to_exclude in [
        item for item in to_exclude if isinstance(item, Path)
    ]:
